...
const (
	...
	// files
	_file_source    = "cc.csv.gz"
	_file_out_nativ = "../cc.go"

	// components
	_header_nativ    = "// package countrycodeint32\n// Code generated : DO NOT EDIT MANUALLY: "
	_package_nativ   = "package countrycodeint32\n\n"
	_map_start_nativ = "// CC ... \nvar CC = map[int32]string{\n"
	_map_end         = "}\n"
)
...
func main() {
		...
		for i := 0; i < worker; i++ {
			go func() {
				for line := range feed_chan {
					...
					cc, err := strconv.ParseInt(s[0], 10, 32)
					if err != nil {
						outSkip("[unable to parse cc string into number, skipping]: ", line)
						continue
					}
					if cc > 9999 {
						outSkip("[number to large, skipping]: ", line)
					}
					co_nativ_chan <- []byte("\t" + I32toS(int32(cc)) + ": \"" + s[1] + "\",\n")
				}
				bg.Done()
			}()
		}
	...
	out("Unique lines processed:      " + strconv.Itoa(counter))
